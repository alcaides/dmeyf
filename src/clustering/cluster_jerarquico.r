...
# baja4 <- dataset2[meses_muerte == 4, ]
# setwd( "~/buckets/b1/" )
# 
# #leo el dataset , aqui se puede usar algun super dataset con Feature Engineering
# dataset  <- dataset2
#   
# #fread( "datasets/semillerio_dataset_lag1.csv.gz", stringsAsFactors= TRUE)
# gc()
...
#############################################################################
# a = dataset[  , mean(ctrx_quarter),  cluster2 ]
# b = dataset[  , sd(ctrx_quarter),  cluster2 ]
# merge(a,b, by="cluster2")
# colnames(a)
# 
# all.vars = c()
# for (var in campos_buenos){
#   print(var)
#   var.mean = dataset[  , mean(unlist(var)),  cluster2 ]
#   colnames(var.mean) = c('cluster2', paste0(var,'mean'))
#   var.std = dataset[  , sd(var),  cluster2 ]
#   colnames(var.std) = c('cluster2', paste0(var, 'std'))
#   var.metrics = merge(var.mean, var.std, by = 'cluster2')
#   all.vars = merge(var.metrics, all.vars, by = 'cluster2')
# }
# var
...
# a = dataset[  , mean(ctrx_quarter),  cluster2 ]
# b = dataset[  , sd(ctrx_quarter),  cluster2 ]
# var.ctrx = merge(a,b, by="cluster2")
# colnames(var.ctrx) = c('cluster2','ctrx_quarter_mean','ctrx_quarter_var')
# var.ctrx
# # 
# # cpayroll_trx
# a = dataset[  , mean(cpayroll_trx),  cluster2 ]
# b = dataset[  , sd(cpayroll_trx),  cluster2 ]
# var.cpayroll = merge(a,b, by="cluster2")
# colnames(var.ctrx) = c('cluster2','payroll_mean','payroll_var')
# var.cpayroll

# #############################################################################

#Calcular Medias y Varianza para cada variable en cada cluster
# cortar <- c("ctrx_quarter", "mcaja_ahorro", "mtarjeta_visa_consumo", "mcuentas_saldo",
